...
const StudentDashboard = () => {
  ...
  const segmentData = useMemo(() => {
    ...
    const counts = segments.reduce((acc: Record<string, any>, seg) => {
      if (!acc[seg.name]) {
  ...
  const COLORS = ['#3B82F6', '#06B6D4', '#F59E0B', '#EF4444', '#8B5CF6', '#10B981'];
  const DARK_COLORS = {
    primary: '#3B82F6',      // Blue
  ...
  const StatCard = ({ title, value, icon: Icon, color, subtitle, trend }: any) => (
    <div className="rounded-lg shadow-lg p-6 border-l-4 transition-all duration-200 hover:shadow-xl hover:scale-[1.02]" 
         style={{
      ...
      <div className="flex items-center justify-between">
        <div>
          <p className="text-sm font-medium" style={{color: DARK_COLORS.textSecondary}}>{title}</p>
          <p className="text-2xl font-bold" style={{color: DARK_COLORS.text}}>{value}</p>
          {subtitle && <p className="text-xs" style={{color: DARK_COLORS.textSecondary}}>{subtitle}</p>}
          {trend && (
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center transition-all duration-300" style={{backgroundColor: DARK_COLORS.background}}>
        <div className="text-center">
          <div className="animate-spin rounded-full h-32 w-32 border-b-2 mx-auto" style={{borderColor: DARK_COLORS.primary}}></div>
          <p className="mt-4 transition-all duration-300" style={{color: DARK_COLORS.textSecondary}}>Loading comprehensive student data...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen transition-all duration-300" style={{backgroundColor: DARK_COLORS.background}}>
      {/* Header */}
      <div className="shadow-lg border-b transition-all duration-300" style={{
        backgroundColor: DARK_COLORS.surface,
        borderColor: DARK_COLORS.border
      }}>
        <div className="px-6 py-4">
          <h1 className="text-2xl font-bold transition-all duration-300" style={{color: DARK_COLORS.text}}>
            Student Performance Analytics Dashboard
          </h1>
          <p className="transition-all duration-300" style={{color: DARK_COLORS.textSecondary}}>
            Comprehensive insights from {studentData.length} student records
          </p>
        </div>
          ...
          <nav className="flex space-x-8">
            {[
              ...
              <button
                ...
                className={`flex items-center px-3 py-2 text-sm font-medium border-b-2 transition-all duration-200 ${
                  activeTab === id
      ...
      <div className="border-b px-6 py-4 transition-all duration-300" style={{
        backgroundColor: DARK_COLORS.surface,
        borderColor: DARK_COLORS.border
      }}>
        <div className="flex flex-wrap gap-4 items-center">
          <div className="flex-1 min-w-64">
            <div className="relative">
              <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 w-4 h-4" style={{color: DARK_COLORS.textSecondary}} />
              <input
                type="text"
          ...
          <select
            ...
            className="px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-400 transition-all duration-200"
            style={{
              backgroundColor: DARK_COLORS.card,
              borderColor: DARK_COLORS.border,
              color: DARK_COLORS.text
          ...
          <div className="flex gap-2">
            <button
              ...
              style={{backgroundColor: DARK_COLORS.success}}
            >
            ...
            <button
              ...
              className="flex items-center px-4 py-2 text-white rounded-lg hover:scale-105 transition-all duration-200"
              style={{backgroundColor: DARK_COLORS.primary}}
            >
              <Download className="w-4 h-4 mr-2" />
              Export JSON
            </button>
        ...
        <div className="mt-3 text-sm transition-all duration-300" style={{color: DARK_COLORS.textSecondary}}>
        ...
        {activeTab === 'overview' && (
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <div className="p-6 rounded-lg shadow-lg transition-all duration-300" style={{
                backgroundColor: DARK_COLORS.card,
                borderColor: DARK_COLORS.border
              }}>
                <h3 className="text-lg font-semibold mb-4 transition-all duration-300" style={{color: DARK_COLORS.text}}>Performance Distribution</h3>
                <ResponsiveContainer width="100%" height={200}>
                  <BarChart data={[
                    ...
                    <CartesianGrid strokeDasharray="3 3" stroke={DARK_COLORS.border} />
                    <XAxis dataKey="range" stroke={DARK_COLORS.textSecondary} />
                    <YAxis stroke={DARK_COLORS.textSecondary} />
                    <Tooltip 
                      contentStyle={{
                        backgroundColor: DARK_COLORS.card,
                    ...
                    />
                    <Bar dataKey="count" fill={DARK_COLORS.primary} />
                  </BarChart>
                </ResponsiveContainer>
              </div>
              ...
              <div className="p-6 rounded-lg shadow-lg transition-all duration-300" style={{
                ...
                <h3 className="text-lg font-semibold mb-4 transition-all duration-300" style={{color: DARK_COLORS.text}}>Age Distribution</h3>
                <ResponsiveContainer width="100%" height={200}>
                  <PieChart>
                    <Pie
            ...
            <div className="p-6 rounded-lg shadow-lg transition-all duration-300" style={{
              ...
              <h3 className="text-lg font-semibold mb-4 transition-all duration-300" style={{color: DARK_COLORS.text}}>Gender Performance Comparison</h3>
              <ResponsiveContainer width="100%" height={300}>
                <BarChart data={genderPerformance}>
                  <CartesianGrid strokeDasharray="3 3" stroke={DARK_COLORS.border} />
                  <XAxis dataKey="gender" stroke={DARK_COLORS.textSecondary} />
                  <YAxis yAxisId="left" stroke={DARK_COLORS.textSecondary} />
                  <YAxis yAxisId="right" orientation="right" stroke={DARK_COLORS.textSecondary} />
                  <Tooltip 
                    contentStyle={{
                      backgroundColor: DARK_COLORS.card,
                      border: `1px solid ${DARK_COLORS.border}`,
                  ...
                  />
                  <Bar yAxisId="left" dataKey="avgScore" fill={DARK_COLORS.primary} name="Average Score" />
                  <Bar yAxisId="right" dataKey="avgStudyHours" fill={DARK_COLORS.secondary} name="Avg Study Hours" />
                </BarChart>
              </ResponsiveContainer>
            </div>
          </div>
        ...
        {activeTab === 'performance' && (
          <div className="space-y-6">
            <div className="p-6 rounded-lg shadow-lg transition-all duration-300" style={{
              ...
              <h3 className="text-lg font-semibold mb-4 transition-all duration-300" style={{color: DARK_COLORS.text}}>Study Hours vs. Exam Performance</h3>
              <ResponsiveContainer width="100%" height={400}>
                <ScatterChart data={studyPerformanceData}>
                  <CartesianGrid stroke={DARK_COLORS.border} />
                  <XAxis dataKey="study_hours" name="Study Hours/Day" stroke={DARK_COLORS.textSecondary} />
                  <YAxis dataKey="exam_score" name="Exam Score" stroke={DARK_COLORS.textSecondary} />
                  ...
                  />
                  <Scatter dataKey="exam_score" fill={DARK_COLORS.primary} />
                </ScatterChart>
              </ResponsiveContainer>
            </div>
            ...
            <div className="p-6 rounded-lg shadow-lg transition-all duration-300" style={{
              ...
              <h3 className="text-lg font-semibold mb-4 transition-all duration-300" style={{color: DARK_COLORS.text}}>Top Performer Profile</h3>
              <ResponsiveContainer width="100%" height={300}>
                <RadarChart data={topPerformerProfile}>
                  <PolarGrid stroke={DARK_COLORS.border} />
                  <PolarAngleAxis dataKey="subject" stroke={DARK_COLORS.textSecondary} />
                  <PolarRadiusAxis angle={90} domain={[0, 100]} stroke={DARK_COLORS.textSecondary} />
                  <Radar name="Top Performers" dataKey="value" stroke={DARK_COLORS.primary} fill={DARK_COLORS.primary} fillOpacity={0.3} />
                </RadarChart>
              </ResponsiveContainer>
            </div>
          </div>
              ...
              <div className="bg-white p-6 rounded-lg shadow-md">
                ...
                <div className="space-y-3">
                  ...
                  <div className="p-3 bg-green-50 rounded-lg">
                    <div className="font-medium text-green-900">Low Risk Students: {filteredData.length - (predictiveInsights.highRiskCount || 0) - (predictiveInsights.mediumRiskCount || 0)}</div>
